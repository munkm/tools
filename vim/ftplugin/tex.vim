...
call IMAP('EFI', "\\begin{figure}[htb]\<cr>\\centering\<cr>\\includegraphics{<+file+>}\<cr>\\caption{<+caption text+>}\<cr>\\label{fig:<+label+>}\<cr>\\end{figure}<++>", "tex")
call IMAP('ESU', '\subfloat[<++>]{%<++>%}%<++>', 'tex')
call IMAP('ETA', "\\begin{table}[htb]\<cr>\\centering\<cr>\\begin{tabular}{<+dimensions+>}\<cr>\\toprule %-------------------------------------------- \<cr><++>\<cr>\\midrule %--------------------------------------------\<cr><++>\<cr>\\bottomrule %-----------------------------------------\<cr>\\end{tabular}\<cr>\\caption{<+Caption text+>}\<cr>\\label{tab:<+label+>}\<cr>\\end{table}<++>", "tex")
...
"verbatim
function! Tex_InsertVerbatim()
  return IMAP_PutTextWithMovement('\verb|<++>|<++>')
endfunction

imap <buffer> <M-v> <C-r>=Tex_InsertVerbatim()<CR>
